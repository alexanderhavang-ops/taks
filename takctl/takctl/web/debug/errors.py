...
from takctl.web.session import get_session, require_session
...
def mount_debug_and_errors(app: FastAPI) -> None:
    @app.middleware("http")
    async def _taks_request_id_and_capture(req: Request, call_next):
    ...
    def _debug_last_exception(req: Request):
        ...
        html = (
            "<!doctype html><html><head><meta charset=\"utf-8\"/><title>last_exception</title></head><body>"
    ...
    async def _taks_handle_starlette_http(req: Request, exc: StarletteHTTPException):
        ...
        if _wants_html(req) and _debug_allowed(req):
            return _taks_error_page(
                ...
                title=str(exc.status_code) + " HTTP error",
    ...
    async def _taks_handle_uncaught(req: Request, exc: Exception):
        ...
        # Public soldier-card paths: show HTML even without auth, but only for that namespace
        ...
        # Normal: only show HTML to authenticated browser-ish clients
        if _wants_html(req) and _debug_allowed(req):
    ...
    # Make HEAD /api/_debug/last_exception behave nicely for smoke checks
    @app.head("/api/_debug/last_exception", include_in_schema=False)
    async def _debug_last_exception_head():
