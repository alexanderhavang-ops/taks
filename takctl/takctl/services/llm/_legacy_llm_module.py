...
def llm_status(_ctx: AppContext | None) -> dict[str, Any]:
    ...
    return {
        ...
        "health": {
            "ok": ok,
            "status_code": (code if should_probe else None) or None,
            "body": body,
...
def llm_plan(ctx: AppContext, data: dict[str, Any], title: str = "LLM Plan") -> RenderPlan:
    ...
    # If heuristic planner is unavailable, degrade loudly but safely.
    if build_tactical_plan is None:
        raise RuntimeError("LLM heuristic planner unavailable: build_tactical_plan import failed")
    ...
    if reachable:
        ...
        if (
            ...
            rp.meta = dict(getattr(rp, "meta", None) or {})
            rp.meta["mode"] = "llm"
            return rp
        ...
        rp.meta["fallback_reason"] = f"llm_plan_failed: http={code or 'n/a'} err={err or 'n/a'}"
        return rp
