...
def _safe_unit_fs(unit_path: str) -> str:
    """
    unit_path is a logical path (may contain / for hierarchy).
    On disk, we store it under state/units/<unit_path>/... so hierarchy is natural.
    """
...
def _copy_tree(src: Path, dst: Path) -> Dict[str, Any]:
    """
    Copy src -> dst (recursive). If src doesn't exist, it's a no-op.
    Returns stats.
    """
...
def _manifest_from_root(root_dir: Path, bundle_root_name: str) -> Dict[str, Any]:
    """
    Build a simple manifest describing layout.
    """
    dirs: List[str] = []
    if root_dir.exists():
    ...
    return {
        ...
        "description": "TAKS bundle built from orchestrator state overlays (role + unit). Receiver decides semantics.",
        "built_utc": _utc_iso(root_dir.stat().st_mtime) if root_dir.exists() else _utc_iso(datetime.now(tz=timezone.utc).timestamp()),
        "layout": {
...
def build_bundle_from_state(
    *,
    ...
    """
    Build a .tar.gz + .manifest.json in bundles_dir().

    Content comes from:
      - roles/<role>/bundle/   (optional base)
      - units/<unit_path>/bundle/ (optional override)

    The on-disk bundle root folder name will be <bundle_base>/ inside the tar.
    """
    up = _safe_unit_fs(unit_path)
    r = (role or "").strip()
    if not r:
    ...
    with tempfile.TemporaryDirectory(prefix="taks-bundle-") as td:
        ...
        # Copy overlays: role first, then unit overrides
