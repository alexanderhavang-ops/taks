    ...
    order.forEach(function(name){
      ...
      if(!arr.length){
        ...
        arr.forEach(function(item){
          const row = S.el('div', { style: 'display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap' });
          row.appendChild(S.el('div', null, S.el('code', { text: item.path || '' })));
          row.appendChild(S.el('div', { className: 'muted', text: item.kind || '' }));
